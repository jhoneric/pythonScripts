...
g_ESRI_variable_7 = u'Estaciones_Categoria'
g_ESRI_variable_8 = u'gdbideam_gdbideam_DOM_Categoria_nom_categoria'
g_ESRI_variable_9 = u'idestadoestaciontm'
g_ESRI_variable_10 = u'ID_EstadoEstacion'
g_ESRI_variable_11 = u'Estaciones_EstEstacion'
g_ESRI_variable_12 = u'gdbideam_gdbideam_DOM_EstadoEstacion_nom_estadoestacion'
g_ESRI_variable_13 = u'idmunicipio'
g_ESRI_variable_14 = u'id_municpio'
g_ESRI_variable_15 = u'Estaciones_Mpio'
g_ESRI_variable_16 = u'gdbideam_gdbideam_DOM_Municipios_nom_municipio'
g_ESRI_variable_17 = u'iddepartamento'
g_ESRI_variable_18 = u'id_depto'
g_ESRI_variable_19 = u'Estaciones_Depto'
g_ESRI_variable_20 = u'gdbideam_gdbideam_DOM_Depto_nom_depto'
g_ESRI_variable_21 = u'idtecnologiatm'
g_ESRI_variable_22 = u'id_tecestacion'
g_ESRI_variable_23 = u'Estaciones_Tecnologia'
g_ESRI_variable_24 = u'gdbideam_gdbideam_DOM_TecEstacion_nom_tecestacion'
g_ESRI_variable_25 = u'nomruleid'
g_ESRI_variable_26 = u'Estaciones_RuleID'
g_ESRI_variable_27 = u'gdbideam_gdbideam_DOM_RuleID_idruleid'
g_ESRI_variable_28 = u'in_memory\\featureTempTwo'
g_ESRI_variable_29 = u'gdbideam_gdbideam_DOM_Categoria_id_categoria'
...
def CalculoAtributos():
    ...
    # copiando a un segundo feature temporal para ejecutar el cursor que calculara los campos creados    
    arcpy.CopyFeatures_management(featureTemp,featureTempTwo)
    
    #fieldlist = arcpy.ListFields(featureTempTwo)
    #for f in fieldlist:
    #   arcpy.AddMessage(f.name)

    # listado de los campos requeridos por el cursor para calcular los campos agregados
    F = [g_ESRI_variable_26, g_ESRI_variable_29, g_ESRI_variable_16, g_ESRI_variable_15, g_ESRI_variable_20, g_ESRI_variable_19, g_ESRI_variable_24, g_ESRI_variable_23, g_ESRI_variable_7, g_ESRI_variable_8, g_ESRI_variable_11, g_ESRI_variable_12]
    # El calculo del campo Rule Id se hace por meio del siguiente diccionario, obtenido de la tabla DOM categoria
    options = {'AM': 2, 'CO': 3, 'CP': 4, 'LG': 5, 'LM': 6, 'MM': 7, 'ME': 8, 'PG': 9, 'PM': 10, 'RS': 11, 'SP': 12, 'SS':13}
    
    with arcpy.da.UpdateCursor(featureTempTwo, F) as cursor1:
        for row in cursor1:
            if row[1] in options:
                row[0] = options[row[1]]
            else:
    ...
    return featureTempTwo

#Función que realiza el borrado y llenado de la capa Estaciones en la base Pruebas/Oficial
def CargaFerature(passed_feature):
    t9 = time.time()
    featureDelete=os.path.join(conSDEOfi,DiccFeature["EstacionesOfi"])
    
    conFeatureAppend="NOMBRE 'NOMBRE' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_nombre,-1,-1;CATEGORIA 'CATEGORIA' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_Categoria,-1,-1;LONGITUD 'LONGITUD' true true false 8 Double 0 0 ,First,#,"+passed_feature+",Estaciones_longitud,-1,-1;LATITUD 'LATITUD' true true false 8 Double 0 0 ,First,#,"+passed_feature+",Estaciones_latitud,-1,-1;ALTITUD 'ALTITUD' true true false 8 Double 0 0 ,First,#,"+passed_feature+",Estaciones_altitud,-1,-1;DEPARTAMEN 'DEPARTAMEN' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_Depto,-1,-1;MUNICIPIO 'MUNICIPIO' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_Mpio,-1,-1;ESTADO_EST 'ESTADO_EST' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_EstEstacion,-1,-1;"+u"TECNOLOG\xcd 'TECNOLOG\xcd' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_Tecnologia,-1,-1;FECHA_INST 'FECHA_INST' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_fechainstalacion,-1,-1;FECHA_SUSP 'FECHA_SUSP' true true false 254 Text 0 0 ,First,#,"+passed_feature+",Estaciones_fechasuspension,-1,-1;RULEID 'RULEID' true true true 4 Long 0 0 ,First,#,"+passed_feature+",Estaciones_RuleID,-1,-1;OVERRIDE 'OVERRIDE' true true true 0 Blob 0 0 ,First,#"
    arcpy.DeleteFeatures_management(featureDelete)
    arcpy.AddMessage(passed_feature)
    arcpy.Append_management(passed_feature,featureDelete,"NO_TEST",conFeatureAppend,"")
...
def actualizarCodigoGdbEnterprise(pCapaFileGdb,pCapaGdbEnterprise):
    t11 = time.time()
    estacionesFgdb = {}
    with arcpy.da.SearchCursor(pCapaFileGdb,["Estaciones_nombre","Estaciones_idestacion"]) as cursor:
        for row in cursor:
...
def main():
    ...
    returned_feature = CalculoAtributos()#Ejecuta la función CalculoAtributos
    CargaFerature(returned_feature)#Ejecuta la función CargaFerature
    actualizarCodigoGdbEnterprise(returned_feature,estacionesOfi)#Ejecuta la función actualizarCodigoGdbEnterprise y se lleva las variables correspondientes a la ubicación de la estacion en base temporal y estación en base oficial
